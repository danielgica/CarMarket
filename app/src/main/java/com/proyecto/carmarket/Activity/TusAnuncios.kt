...
class TusAnuncios : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        volverMenu = findViewById(R.id.tusAnuncios_atras)
        ...
        val localidadEditText = findViewById<EditText>(R.id.tusAnuncios_textoLocalidad)
        ...
        volverMenu.setOnClickListener {
            val progressBar = findViewById<View>(R.id.tusAnuncios_progressBarMarcas)
    ...
    private fun cargaListaMarcas() {
        ...
        db.collection("marcas").get().addOnCompleteListener { task ->
            if (task.isSuccessful) {
                ...
                findViewById<RecyclerView>(R.id.tusAnuncios_recyclerViewMarcas).apply {
                    layoutManager = LinearLayoutManager(this@TusAnuncios, LinearLayoutManager.HORIZONTAL, false)
                    adapter = adaptador
                }
                progressBar.visibility = View.GONE
            } else {
                Log.d("TusAnuncios", "Error obteniendo documentos: ", task.exception)
                progressBar.visibility = View.GONE
            }
        }
    }
    ...
    private fun cargaAnunciosPorMarcaYLocalidad(marca: String, localidad: String) {
        val progressBar = findViewById<View>(R.id.tusAnuncios_progressBarAnuncios)
        progressBar.visibility = View.VISIBLE
        ...
        consulta.whereEqualTo("propietario", MainActivity.email).get().addOnCompleteListener { task ->
            if (task.isSuccessful) {
                ...
                for (document in documentosFiltrados) {
                    ...
                    storageReference.child(carpetaFotos).listAll().addOnSuccessListener { listResult ->
                        ...
                        Log.e("TusAnuncios", "Error al listar fotos en la carpeta: $carpetaFotos", it)
        ...
        val tituloTextView = findViewById<TextView>(R.id.tusAnuncios_tituloQueVes)
    ...
    private fun configurarRecyclerView(listaAnuncios: List<Anuncio>) {
        val adaptador = AdaptadorListaAnuncios(this, listaAnuncios, TuAnuncio::class.java)
        findViewById<RecyclerView>(R.id.tusAnuncios_recyclerViewAnuncios).apply {
            layoutManager = GridLayoutManager(this@TusAnuncios, 2)
            adapter = adaptador
        }
    }
